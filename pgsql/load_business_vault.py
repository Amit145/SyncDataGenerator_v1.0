...
CREATE TABLE {SCHEMA}.bv_address_master (
...
CREATE TABLE {SCHEMA}.bv_address_master_xref (
...
CREATE TABLE {SCHEMA}.bv_home_master (
...
CREATE TABLE {SCHEMA}.bv_home_master_xref (
...
CREATE TABLE {SCHEMA}.bv_legal_person_master (
...
CREATE TABLE {SCHEMA}.bv_legal_person_master_xref (
    global_person_identifier TEXT NOT NULL,
    rawdv_source_name TEXT,
    rawdv_source_business_key TEXT,
    rawdv_hashkey TEXT,
...
CREATE TABLE {SCHEMA}.bv_motor_master (
...
CREATE TABLE {SCHEMA}.bv_motor_master_xref (
...
CREATE TABLE {SCHEMA}.bv_natural_person_master (
    global_person_identifier TEXT PRIMARY KEY,
...
CREATE TABLE {SCHEMA}.bv_natural_person_master_xref (
    global_person_identifier TEXT NOT NULL,
    rawdv_source_name TEXT,
    rawdv_source_business_key TEXT,
    rawdv_hashkey TEXT,
...
CREATE TABLE {SCHEMA}.bv_product_master (
...
CREATE TABLE {SCHEMA}.bv_product_master_xref (
    global_product_identifier TEXT NOT NULL,
    rawdv_source_name TEXT,
    rawdv_source_business_key TEXT,
    rawdv_hashkey TEXT,
...
LOAD_SQL = [
    f"""
    INSERT INTO {SCHEMA}.bv_product_master
    ...
    """,
    f"""
    INSERT INTO {SCHEMA}.bv_product_master_xref
    WITH product_match AS (
        SELECT DISTINCT
            concat(hcrm.product_id, '||', hsap.product_id) AS global_product_identifier,
    ...
    """,
    f"""
    INSERT INTO {SCHEMA}.bv_natural_person_master
    {COMMON_CTES},
    natural_rows AS (
    ...
    """,
    f"""
    INSERT INTO {SCHEMA}.bv_natural_person_master_xref
    {COMMON_CTES}
    ...
    """,
    f"""
    INSERT INTO {SCHEMA}.bv_legal_person_master
    {COMMON_CTES},
    legal_rows AS (
    ...
    """,
    f"""
    INSERT INTO {SCHEMA}.bv_legal_person_master_xref
    {COMMON_CTES}
    ...
    """,
    f"""
    INSERT INTO {SCHEMA}.bv_address_master
    {COMMON_CTES},
    address_rows AS (
    ...
    """,
    f"""
    INSERT INTO {SCHEMA}.bv_address_master_xref
    {COMMON_CTES},
    address_rows AS (
        SELECT DISTINCT
            concat(hac.address_id, '||', hasp.address_id) AS global_address_identifier,
    ...
    """,
    f"""
    INSERT INTO {SCHEMA}.bv_home_master
    ...
    """,
    f"""
    INSERT INTO {SCHEMA}.bv_home_master_xref
    ...
    """,
    f"""
    INSERT INTO {SCHEMA}.bv_motor_master
    WITH latest_policy_product AS (
        SELECT * FROM (
            SELECT lpp.*, row_number() OVER (PARTITION BY policy_hash_key ORDER BY load_date DESC) AS rn
            FROM raw_vault.link_policy_product lpp
    ...
    """,
    f"""
    INSERT INTO {SCHEMA}.bv_motor_master_xref
...
def main() -> None:
    ...
    print(f"Loaded {len(counts)} tables into schema {SCHEMA}:")

...
DROP SCHEMA IF EXISTS business_dimensional CASCADE;
CREATE SCHEMA business_dimensional;
SET search_path TO business_dimensional, business_vault, raw_vault, public;
...
WITH natural_person AS (
    ...
    LEFT JOIN business_dimensional.dim_geography dg
    ...
    LEFT JOIN business_dimensional.dim_identity di ON di.identities_hash_key = lpi.identities_hash_key
...
LEFT JOIN business_dimensional.dim_product dp
...
LEFT JOIN business_dimensional.dim_channel dch ON dch.channel_hash_key = lpc.channel_hash_key
...
LEFT JOIN business_dimensional.dim_insured_object dio ON dio.insured_object_hash_key = lpio.insured_object_hash_key
LEFT JOIN raw_vault.link_insured_object_home lioh ON lioh.insured_object_hash_key = lpio.insured_object_hash_key
LEFT JOIN business_dimensional.dim_home dh
...
LEFT JOIN business_dimensional.dim_motor dm
...
person_marketing_one AS (
    ...
    FROM business_dimensional.dim_marketing
...
LEFT JOIN business_dimensional.dim_person dp
    ON dp.crm_person_hash_key = lpl.person_hash_key OR dp.sap_person_hash_key = lpl.person_hash_key
LEFT JOIN business_dimensional.dim_date dd ON dd.full_date = NULLIF(sl.converted_date, '')::date
...
LEFT JOIN latest_sat sp ON sp.policy_hash_key = hp.policy_hash_key
LEFT JOIN business_dimensional.dim_policy dpol ON dpol.policy_hash_key = hp.policy_hash_key
LEFT JOIN business_dimensional.dim_customer dc ON dc.customer_hash_key = lpcu.customer_hash_key
...
LEFT JOIN business_dimensional.dim_account da ON da.account_hash_key = lpa.account_hash_key
LEFT JOIN business_dimensional.dim_date dd ON dd.full_date = coalesce(NULLIF(sp.transaction_date, '')::date, NULLIF(sp.policy_start_date, '')::date)
...
LEFT JOIN business_dimensional.dim_channel dch ON dch.channel_hash_key = lpch.channel_hash_key
LEFT JOIN policy_broker_one lpb ON lpb.policy_hash_key = hp.policy_hash_key
LEFT JOIN business_dimensional.dim_broker db ON db.broker_hash_key = lpb.broker_hash_key
LEFT JOIN policy_claim_one lclp ON lclp.policy_hash_key = hp.policy_hash_key
LEFT JOIN business_dimensional.dim_claim dcl ON dcl.claim_hash_key = lclp.claim_hash_key
LEFT JOIN policy_override_one lpo ON lpo.policy_hash_key = hp.policy_hash_key
LEFT JOIN business_dimensional.dim_override dov ON dov.override_hash_key = lpo.override_hash_key
LEFT JOIN policy_object_one po ON po.policy_hash_key = hp.policy_hash_key
LEFT JOIN business_dimensional.dim_insured_object dio ON dio.insured_object_hash_key = po.insured_object_hash_key
LEFT JOIN business_dimensional.dim_home dh
    ON split_part(dh.policy_identifier, '||', 1) = hp.policy_id
LEFT JOIN business_dimensional.dim_motor dmo
...
LEFT JOIN business_dimensional.dim_customer dc ON dc.customer_hash_key = pc.customer_hash_key
LEFT JOIN person_account_one pa ON pa.person_hash_key = lqp.person_hash_key
LEFT JOIN business_dimensional.dim_account da ON da.account_hash_key = pa.account_hash_key
LEFT JOIN person_marketing_one pm ON pm.person_hash_key = lqp.person_hash_key
LEFT JOIN business_dimensional.dim_date dd ON dd.full_date = NULLIF(sq.quote_date, '')::date
LEFT JOIN person_campaign_one pcam ON pcam.person_hash_key = lqp.person_hash_key
LEFT JOIN business_dimensional.dim_campaign dcam ON dcam.campaign_hash_key = pcam.campaign_hash_key
LEFT JOIN quote_channel_one lqch ON lqch.quote_hash_key = hq.quote_hash_key
LEFT JOIN business_dimensional.dim_channel dch ON dch.channel_hash_key = lqch.channel_hash_key
LEFT JOIN quote_broker_one lqb ON lqb.quote_hash_key = hq.quote_hash_key
LEFT JOIN business_dimensional.dim_broker db ON db.broker_hash_key = lqb.broker_hash_key
...
LEFT JOIN business_dimensional.dim_customer dc ON dc.customer_hash_key = pcust.customer_hash_key
...
LEFT JOIN business_dimensional.dim_regulation dr ON dr.regulation_hash_key = cr.regulation_hash_key
LEFT JOIN business_dimensional.dim_channel dch ON lower(dch.channel_name) = lower(sc.complaint_channel)
LEFT JOIN business_dimensional.dim_date dd ON dd.full_date = NULLIF(sc.complaint_date, '')::date
LEFT JOIN policy_object_one po ON po.policy_hash_key = hp.policy_hash_key
LEFT JOIN business_dimensional.dim_insured_object dio ON dio.insured_object_hash_key = po.insured_object_hash_key
LEFT JOIN business_dimensional.dim_home dh
    ON split_part(dh.policy_identifier, '||', 1) = hp.policy_id

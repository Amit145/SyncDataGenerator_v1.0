...
The generated YAML contracts follow the Open Data Contract Standard pattern using the local ODCS v3.1.0 schema file:

```text
data_contract/odcs-v3.1.0.schema_ODCS.json
```

The generator validates each contract against this schema before writing the YAML file. This keeps the contract structure aligned to ODCS fields such as:
...
Some project/framework-specific metadata is stored inside ODCS `customProperties`. This is intentional. It allows the contracts to remain ODCS-compatible while still carrying metadata needed by our ETL validation framework and Databricks notebook, such as:

- `sourceFeed`
- `pipelineLayer`
- `expectedFiles`
- `notebookValidationConfig`
- `dqRuleCatalog`
- `allowNoPrimaryKey`
- `referencedColumns`
- `failureAction`
...
The current implementation can generate and verify these contracts automatically from `main.py` when the matching data outputs are produced:
...
Generated contracts are written under:

```text
data_contract/generated/
```

Validation reports are written under:

```text
data_contract/reports/
```

These generated folders are ignored by Git because they are runtime outputs. The source of truth is:

- `data_contract/contract_run_config.yaml`
- files under `data_contract/tools/`
- `data_contract/odcs-v3.1.0.schema_ODCS.json`
- raw data generated by `main.py`
...
data_contract/
  ...
  tools/
    ...
    enhanced_prd02_contract_config.csv
...
CSV config files are also supported for easier editing in Excel. YAML remains the normal/default format, but if the generator is called with a YAML config and a same-name CSV file exists beside it, the CSV is used automatically.

Example:

```text
data_contract/tools/enhanced_prd01_contract_config.yaml
data_contract/tools/enhanced_prd01_contract_config.csv
```

This command will use the CSV file if it exists:

```powershell
--config .\data_contract\tools\enhanced_prd01_contract_config.yaml
```

You can also pass the CSV directly:

```powershell
--config .\data_contract\tools\enhanced_prd01_contract_config.csv
```

Enhanced PRD1 has a dedicated config because some `vault_ready_28` files are sparse grouped extracts:

```text
data_contract/tools/enhanced_prd01_contract_config.yaml
```

Enhanced PRD2 also has a dedicated config for the SAP/source-2 file keys:
...
During the run, `main.py` prints each contract execution:
...
Each entry in `data_contract/contract_run_config.yaml` controls:

- `enabled`: whether that contract should run.
- `inputFrom`: which `main.py` output variable supplies the source folder.
- `domain`, `layer`, `sourceFeed`, `sourceSystem`: metadata written into the contract.
- `contractId`, `contractName`: stable contract identity.
...
If verification finds critical errors and `failMainOnContractError: true`, `main.py` raises an error and stops. Set it to `false` only when you want report-only behavior during development.

Manual flow for a one-off contract:

```text
1. Create or update a YAML/CSV config file with key overrides and custom rules.
...
Use this to run without data contracts:

```powershell
.\venv\Scripts\python.exe .\main.py --skip-data-contracts
```

Use this to run with a different contract run config:

```powershell
.\venv\Scripts\python.exe .\main.py --data-contract-config .\data_contract\contract_run_config.yaml
```
...
For grouped/sparse extracts that do not have one universal physical primary key column, set:

```yaml
tables:
  enhanced_enrichments.csv:
    allowNoPrimaryKey: true
    grain: "Sparse grouped enrichment rows across multiple extract types."
```

Use this only when the source file genuinely has no stable physical key. The verifier and notebook will skip primary key enforcement for that object, but schema, required-field, row-count, and custom quality rules still run.

## CSV Config Format

Use one row per config item. The `record_type` column controls how the row is interpreted:

- `contract`: contract-level metadata.
- `table`: table/file-level overrides.
- `column`: column-level overrides.
- `table_rule`: table-level DQ rule.
- `column_rule`: column-level DQ rule.

Common CSV columns:

```text
record_type,table,column,key,value,primaryKey,businessKey,businessName,grain,allowNoPrimaryKey,required,allowedValues,minValue,maxValue,ruleId,ruleName,type,query,metric,mustBe,dimension,severity,description,referencedColumns,failureAction
```

Example rows:

```csv
record_type,table,column,key,value,primaryKey,businessKey,businessName,grain,allowNoPrimaryKey,required,allowedValues,minValue,ruleId,ruleName,type,query,metric,mustBe,dimension,severity,description,referencedColumns,failureAction
contract,,,purpose,Validate raw claims before ingestion.,,,,,,,,,,,,,,,,,,
table,claim_register.csv,,,,claim_identifier,claim_identifier,Claim Register,One row per claim_identifier per batch.,,,,,,,,,,,,,
column,claim_register.csv,claim_status,,,,,,,,true,Open|Closed|Reopened,,,,,,,,,,,
table_rule,claim_register.csv,,,,,,,,,,,,dq_claim_amounts_non_negative,Claim amounts non-negative,sql,"SELECT COUNT(*) AS invalid_amounts FROM {full_table_name} WHERE claim_requested_amount < 0 AND {target_watermak_exp}",,"{""invalid_amounts"": 0}",accuracy,error,Claim amount must not be negative.,claim_requested_amount,reject
```

Use `|` for list values such as composite keys, allowed values, and referenced columns:

```csv
table,customer_lead_bridge.csv,,,,customer_ref|lead_ref,customer_ref|lead_ref
```

Use JSON for mapped values such as `mustBe`:
...
.\venv\Scripts\python.exe .\data_contract\tools\create_data_contract.py `
  ...
  --config <optional-config-yaml-or-csv> `
...
- `--config`: optional YAML or CSV file with key overrides and custom rules.
...
Example for Enhanced PRD1 `vault_ready_28`:

```powershell
.\venv\Scripts\python.exe .\data_contract\tools\create_data_contract.py `
  --input-path .\data\raw\enhanced\prd_01\20260730184046\vault_ready_28 `
  --domain enhanced `
  --layer raw `
  --source-feed prd_01 `
  --source-system "Enhanced source-1 vault_ready_28" `
  --contract-id enhanced-prd-01-raw-0001 `
  --contract-name "Enhanced PRD 01 Raw Source Feed" `
  --config .\data_contract\tools\enhanced_prd01_contract_config.yaml `
  --output .\data_contract\enhanced\raw\prd_01\enhanced_prd_01_raw_ODCS.yaml
```

Example for Enhanced PRD2:

```powershell
.\venv\Scripts\python.exe .\data_contract\tools\create_data_contract.py `
  --input-path .\data\raw\enhanced\prd_02\20260730184046 `
  --domain enhanced `
  --layer raw `
  --source-feed prd_02 `
  --source-system "Enhanced SAP source-2" `
  --contract-id enhanced-prd-02-raw-0001 `
  --contract-name "Enhanced PRD 02 Raw Source Feed" `
  --config .\data_contract\tools\enhanced_prd02_contract_config.yaml `
  --output .\data_contract\enhanced\raw\prd_02\enhanced_prd_02_raw_ODCS.yaml
```

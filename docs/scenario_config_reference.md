...
- `generate_prd_raw`: When `true`, normal `main.py` writes mode-scoped product raw folders under `data/raw/base/prd_01/<run_id>`, `data/raw/base/prd_02/<run_id>`, `data/raw/enhanced/prd_01/<run_id>`, `data/raw/enhanced/prd_02/<run_id>`, `data/raw/enhanced/raw_vault/<run_id>`, `data/raw/mlops/prd_01/<run_id>`, and `data/raw/mlops/prd_02/<run_id>`. Enhanced add-on source tables are staged as unprefixed files under `data/raw/enhanced/prd_01/<run_id>/addons`, packaged into the flat enhanced source-1 deliverable `data/raw/enhanced/prd_01/<run_id>/vault_ready_28`, and consolidated with the five enhanced SAP source-2 files into the flat 32-file `data/raw/enhanced/raw_vault/<run_id>` package. The enhanced folder name remains `vault_ready_28`, but the active contract is 27 CSV files because standalone insured-object raw is derived from bridge files.
- `generate_prd_silver`: When `true`, normal `main.py` rebuilds mode-scoped silver vault folders under `data/silver/base/<run_id>`, `data/silver/enhanced/<run_id>`, and `data/silver/mlops/<run_id>`. This requires `generate_prd_raw=true` for the same run. Enhanced silver is rebuilt from enhanced PRD1 `vault_ready_28`; MLOps silver is rebuilt from MLOps PRD1 plus MLOps `prd_delta`.
...
- `generate_claims_product`: When `true`, normal `main.py` writes the optional claims product stack. Raw outputs are `data/raw/claims/<run_id>/raw`, claims CRM/source-1 `prd_01`, claims SAP/source-2 `prd_02`, and claims `raw_vault`. The PRD1/PRD2 split is driven by `claims/Claims_2Sources_DataTables.xlsx`; `raw_vault` is the consolidated 23-file claims LDM raw input used to create claims bronze, silver vault, and gold.
- `generate_policy_product`: When `true`, normal `main.py` writes the optional policy product raw split under `data/raw/policy/<run_id>/prd_01` and `data/raw/policy/<run_id>/prd_02`. PRD1 is the CRM/full policy logical model from the policy PDF; PRD2 is the SAP/source-2 subset from `policy_prd_inputs/nPolicy_BV_Mapping_Rules.xlsx` and emits `policy.csv`, `coverage.csv`, and `policy_coverage.csv`.

...
- `generators/raw_prd_generator.py`: writes PRD1 as a folder-level CRM raw split, SAP PRD2 for base/MLOps Business Vault source-2, MLOps product delta raw, and the enhanced `vault_ready_28` package
...
- PRD2 outputs: `data/raw/base/prd_02/<run_id>`, `data/raw/mlops/prd_02/<run_id>`
...
- PRD1 CRM `product_catalog.csv` includes `product_type_txt`, `underwriting_group_txt`, `regulatory_approval_cd`, `product_status_txt`, `product_lob_cd`, and `product_launch_dt`. `product_type_txt` equals `product_cd`; codes follow `RAC###` and `LOB###`; launch date is before the earliest policy start date for the product.
- Enhanced PRD1 stages unprefixed source-style entity, bridge, and enrichment extracts under `addons/`, then writes the final flat 28-file `vault_ready_28` package used to rebuild enhanced vault relationships and added satellite attributes.

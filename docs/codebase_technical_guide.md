...
- `generators/raw_prd_generator.py`: writes PRD1 as a folder-level CRM raw split and PRD2 as source-style raw files for enhanced/MLOps additions
...
- `misc/build_product_combined_vault.py`: rebuilds a combined vault from base silver/synthetic plus PRD2 raw; normal `main.py` uses it for `data/silver/enhanced/<run_id>` and `data/silver/mlops/<run_id>`
...
- `data/silver/enhanced/<run_id>` from enhanced PRD1 + PRD2 raw
...
- PRD2 outputs: `data/raw/enhanced/prd_02/<run_id>`, `data/raw/mlops/prd_02/<run_id>`
...
- PRD2 contains seven added source-style entity registers: `broker_book.csv`, `campaign_register.csv`, `channel_catalog.csv`, `complaint_register.csv`, `insured_object_register.csv`, `override_register.csv`, and `regulation_register.csv`.
- PRD2 also contains source-style bridge and enrichment extracts needed to rebuild enhanced/MLOps vault relationships and added satellite attributes.
- PRD2 raw columns use `src_*` source headers. The product-combined builder maps those fields back to MLOps vault columns.
- PRD2 must not contain vault-shaped `hub_`, `link_`, or `sat_` files.
- The verifier reconciles PRD2 to the corresponding MLOps added entities and relationships.

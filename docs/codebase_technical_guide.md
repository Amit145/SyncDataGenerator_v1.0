...
Generate only MLOps synthetic output:

```powershell
$env:PYTHONUTF8='1'
python .\main.py --mlops-only
```

`--mlops-only` still builds the base context in memory because MLOps tables depend on the same base people, quotes, policies, products, assets, links, and satellites. It writes only `data/synthetic/mlops/<run_id>` and skips base CSV output, enhanced synthetic output, PRD raw, silver, and SCD2 outputs.
...
- `data/silver`
- `data/scd2`
- optional `data/new_outputs_src` when both `--include-raw-silver` and `--include-new-outputs-src` are passed
...
- `generators/raw_prd_generator.py`: writes PRD1 as a folder-level CRM raw split, SAP PRD2 for base/enhanced/MLOps Business Vault source-2, MLOps product delta raw, the enhanced `vault_ready_28` package, and enhanced consolidated `raw_vault`
- `misc/raw_to_silver_sample.py`: rebuilds base silver vault files from PRD1 raw source fields under `data/raw/base/prd_01/<run_id>`
- `misc/build_product_combined_vault.py`: rebuilds combined vault outputs from base-shaped silver/synthetic plus enhanced `vault_ready_28` or MLOps `prd_delta`; normal `main.py` uses it for `data/silver/enhanced/<run_id>` and `data/silver/mlops/<run_id>`
- `verify_csv.py`: accepts an explicit folder path and can run base-style business/date/churn checks against `data/product_combined/<run_id>`
...
- `generators/claim_ldm_generator.py`: writes optional claims LDM raw, claims PRD1/PRD2 source views from `claims/Claims_2Sources_DataTables.xlsx`, and claims `raw_vault`
...
- `data/silver/base/<run_id>` from PRD1 raw
- `data/silver/enhanced/<run_id>` from enhanced PRD1 `vault_ready_28`
- `data/silver/mlops/<run_id>` from MLOps PRD1 + PRD2 raw
- `data/silver/rebuild/<run_id>`
- `data/silver/api/<run_id>`
- `data/silver/claims/<run_id>`
- `data/silver/data_source/<run_id>`

The `silver/base`, `silver/enhanced`, and `silver/mlops` folders are generated by normal `main.py` only when `output_settings.generate_prd_raw=true` and `output_settings.generate_prd_silver=true` in `config/scenario_v1.json`. The `silver/rebuild`, `silver/api`, `silver/claims`, and `silver/data_source` folders are legacy/optional raw-source rebuilds generated only by `output_settings.generate_legacy_raw_silver=true`, `--include-raw-silver`, or follow-up silver tools.

Enhanced and MLOps silver rebuilds use their own PRD1 source folders first. `main.py` rebuilds enhanced from the final 28-file package at `data/raw/enhanced/prd_01/<run_id>/vault_ready_28`, which contains CRM foundation files, enhanced entity files, and flat relationship/enrichment group files. Enhanced also writes SAP/source-2 under `data/raw/enhanced/prd_02/<run_id>` and a consolidated upload package under `data/raw/enhanced/raw_vault/<run_id>`. MLOps rebuilds a base-shaped intermediate vault from `data/raw/mlops/prd_01/<run_id>` and applies the matching `prd_delta` bridge, entity, and enrichment extracts.
...
10. Write intermediate base output to `data/output/<run_id>`.
11. Build `base_context`, which is reused by raw and enhanced generation.
12. If `output_settings.generate_prd_raw=true`, generate mode-scoped PRD1 raw for base, enhanced, and MLOps.
13. If `--include-raw-silver` is passed, map raw CRM to canonical raw.
14. If `--include-raw-silver` is passed, generate independent raw API context and raw API.
15. If `--include-raw-silver` is passed, generate independent raw claims context and raw claims.
16. If `--include-raw-silver` is passed, generate independent `data_source` context and data source raw extracts.
17. If `--include-raw-silver` is passed, map data source raw to canonical raw.
18. If `--include-raw-silver` is passed, generate raw SCD2 for eligible raw sources.
19. Generate optional `new_outputs_src` source-specific outputs and SCD2 only when both `--include-raw-silver` and `--include-new-outputs-src` are passed.
20. If `--include-raw-silver` is passed, build API silver.
21. Generate enhanced synthetic output.
22. Generate MLOps synthetic output.
23. If `output_settings.generate_prd_raw=true`, generate source-style PRD2 raw from enhanced/MLOps added entities, relationships, and enrichment deltas.
24. If `output_settings.generate_prd_silver=true`, rebuild base/enhanced/MLOps silver from PRD raw.
25. Generate enhanced/MLOps SCD2 if prior comparable history exists.
26. Validate base output structure and integrity.
27. Normalize base output into `data/synthetic/base/<run_id>`.
28. Generate base SCD2 if prior base history exists.
29. Keep intermediate `data/output/<run_id>` unless `--remove-working-output` is passed.
...
- the normal `main.py` path now focuses on synthetic base, enhanced, MLOps, synthetic SCD2 when history exists, and detailed validation. Mode-scoped PRD raw and PRD silver are controlled by `output_settings.generate_prd_raw` and `output_settings.generate_prd_silver`. Legacy raw/canonical/silver outputs require `output_settings.generate_legacy_raw_silver=true` or `--include-raw-silver`, and `new_outputs_src` additionally requires `output_settings.generate_new_outputs_src=true` or `--include-new-outputs-src`.
...
- generated by `generators/raw_prd_generator.py`
- PRD1 outputs: `data/raw/base/prd_01/<run_id>`, `data/raw/enhanced/prd_01/<run_id>`, `data/raw/mlops/prd_01/<run_id>`
- PRD2 outputs: `data/raw/base/prd_02/<run_id>`, `data/raw/enhanced/prd_02/<run_id>`, `data/raw/mlops/prd_02/<run_id>`
- enhanced raw-vault output: `data/raw/enhanced/raw_vault/<run_id>`
- `prd_delta` outputs: `data/raw/mlops/prd_delta/<run_id>`
- PRD1 preserves the existing CRM raw file shape.
- PRD2 preserves the SAP/source-2 raw file shape from `business_vault/bv.xlsx`: base/MLOps include `person.csv`, `address.csv`, `product.csv`, `home.csv`, `motor.csv`, and `insured_object.csv`; enhanced includes `person.csv`, `address.csv`, `product.csv`, `home.csv`, and `motor.csv` only.
...
- PRD1 CRM `product_catalog.csv` includes `product_type_txt`, `product_variant`, `underwriting_group_txt`, `regulatory_approval_cd`, `product_status_txt`, `product_lob_cd`, and `product_launch_dt`. `product_type_txt` equals `product_cd`; `product_variant` equals `product_line`; codes follow `RAC###` and `LOB###`; launch date is before the earliest policy start date for the product.
- PRD1 CRM `vehicle_asset.csv` includes `driver_experience_years`, derived from linked natural-person birth date as `max(age - 17, 0)` and capped to a realistic range.
- Enhanced PRD1 `property_asset.csv` and `vehicle_asset.csv` include asset-level insured-object columns: `insured_object_id`, `insured_object_type`, `insured_object_sub_type`, `insured_object_description`, `insured_value`, `currency_code`, `insured_object_start_date`, `insured_object_end_date`, and `insured_object_current_status`. These values are aligned to policy/home/motor insured-object bridge extracts and do not require a standalone insured-object register.
- Enhanced PRD1 stages unprefixed source-style entity, bridge, and enrichment extracts under `addons/`, then writes the final flat `vault_ready_28` package used to rebuild enhanced vault relationships and added satellite attributes. The folder name is retained for compatibility, but the active contract is 27 CSV files because explicit insured-object raw is derived from the bridge files. Enhanced `raw_vault` is a flat 32-file package: those 27 source-1 files plus the five SAP/source-2 files from enhanced `prd_02`.
- Enhanced raw 28 includes raw-only NPS/feedback source extensions for modelling: customer onboarding satisfaction/feedback on `customer_portfolio.csv`, complaint satisfaction/feedback on `complaint_register.csv`, and claim experience fields on `claim_register.csv` (`src_is_fault_claim`, `src_claim_satisfaction_score`, `src_claims_feedback`, `src_is_claim_complaint_raised`). Claim complaint raised is derived through claim-policy and complaint-policy links so the raw package remains vault-consolidation safe.
- MLOps `prd_delta` contains seven added source-style entity registers: `broker_book.csv`, `campaign_register.csv`, `channel_catalog.csv`, `complaint_register.csv`, `insured_object_register.csv`, `override_register.csv`, and `regulation_register.csv`.
- MLOps `prd_delta` also contains source-style bridge and enrichment extracts needed to rebuild MLOps vault relationships and added satellite attributes.
- `prd_delta` raw columns use `src_*` source headers. The product-combined builder maps those fields back to MLOps vault columns.
...
- legacy independent raw claims are generated by `generators/raw_claims_generator.py`
- claims product raw is generated by `generators/claim_ldm_generator.py` when `--include-claims-product` or `output_settings.generate_claims_product=true` is used
- claims product writes `data/raw/claims/<run_id>/raw`, full-LDM source `prd_01`, workbook source-2 `prd_02`, and consolidated `raw_vault`
- `prd_01` contains all 23 claims LDM raw files; `prd_02` is the SAP/source-2 view from `claims/Claims_2Sources_DataTables.xlsx`
- current workbook-driven split covers `claim.csv`, `loss_event.csv`, and `claim_investigation.csv`
- `raw_vault` reconstructs the existing 23-file claims LDM raw shape and is the input to claims bronze/silver/gold
- PRD1 and PRD2 are traceable by row position and business values back to the same raw-vault records for the workbook split tables; `_source_manifest.csv` is informational only and is not required for loading
- verifier: `misc/verify_claims_two_source_raw.py`
- uses independent synthetic context

...
- PRD2 preserves the SAP/source-2 raw file shape from `business_vault/bv.xlsx`: `person.csv`, `address.csv`, `product.csv`, `home.csv`, `motor.csv`, and `insured_object.csv`.
...
- Enhanced PRD1 stages unprefixed source-style entity, bridge, and enrichment extracts under `addons/`, then writes the final flat 28-file `vault_ready_28` package used to rebuild enhanced vault relationships and added satellite attributes. Enhanced `raw_vault` is a flat 34-file package: those 28 source-1 files plus the six SAP/source-2 files from enhanced `prd_02`.
...
- claims product writes `data/raw/claims/<run_id>/raw`, source-1 `prd_01`, source-2 `prd_02`, and consolidated `raw_vault`
- `prd_01` is the CRM/source-1 view and `prd_02` is the SAP/source-2 view from `claims/Claims_2Sources_DataTables.xlsx`
...
- PRD1 and PRD2 are traceable by row position and business values back to the same raw-vault records; `_source_manifest.csv` is informational only and is not required for loading

# SQL Difference Summary

Generated on 2026-07-17 15:55:23.

This ignores product id and vault object id differences. Tables are matched by `target_table`; the focus is only `build_sql`.
...
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `account_hash_key` source `md5(concat(account_ref, origin_sys))` vs notebook `md5(concat(origin_sys, account_ref))`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`.

<details>
<summary>Raw SQL diff</summary>
...
- FROM tables differ: source uses crm_address_book_csv, sap_address_db; notebook uses combined, crm_address_book_csv, crm_enhanced_address_book_csv, sap_address_db.
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `address_hash_key` source `md5(concat(address_id, origin_sys))` vs notebook `md5(concat(origin_sys, address_id))`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`.
...
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `broker_hash_key` source `md5(concat(src_agent_id, src_system))` vs notebook `md5(concat(origin_sys, src_agent_id))`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`; `record_source` source `src_system` vs notebook `origin_sys`.
...
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `campaign_hash_key` source `md5(concat(src_campaign_id, src_system))` vs notebook `md5(concat(origin_sys, src_campaign_id))`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`; `record_source` source `src_system` vs notebook `origin_sys`.
...
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `channel_hash_key` source `md5(concat(src_channel_id, src_system))` vs notebook `md5(concat(origin_sys, src_channel_id))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `record_source` source `src_system` vs notebook `origin_sys`.
...
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `claim_hash_key` source `md5(concat(src_claim_id, src_system))` vs notebook `md5(concat(origin_sys, src_claim_id))`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`; `record_source` source `src_system` vs notebook `origin_sys`.
...
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `complaint_hash_key` source `md5(concat(src_complaint_id, src_system))` vs notebook `md5(concat(origin_sys, src_complaint_id))`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`; `record_source` source `src_system` vs notebook `origin_sys`.
...
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `consent_hash_key` source `md5(concat(consent_ref, origin_sys))` vs notebook `md5(concat(origin_sys, consent_ref))`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`.
...
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `contact_hash_key` source `md5(concat(contact_ref, origin_sys))` vs notebook `md5(concat(origin_sys, contact_ref))`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`.
...
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `customer_hash_key` source `md5(concat(customer_ref, origin_sys))` vs notebook `md5(concat(origin_sys, customer_ref))`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`.
...
- FROM tables differ: source uses crm_property_asset_csv, sap_home_db; notebook uses combined, crm_property_asset_csv, sap_home_db.
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `home_hash_key` source `md5(concat(home_id, origin_sys))` vs notebook `md5(concat(origin_sys, home_id))`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`.
...
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `identities_hash_key` source `md5(concat(identity_ref, origin_sys))` vs notebook `md5(concat(origin_sys, identity_ref))`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`.
...
- FROM tables differ: source uses crm_property_asset_csv, crm_vehicle_asset_csv; notebook uses combined, crm_property_asset_csv, crm_vehicle_asset_csv.
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `insured_object_hash_key` source `md5(concat(insured_object_id, origin_sys))` vs notebook `md5(concat(origin_sys, insured_object_id))`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`.
...
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `lead_hash_key` source `md5(concat(lead_ref, origin_sys))` vs notebook `md5(concat(origin_sys, lead_ref))`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`.
...
- FROM tables differ: source uses crm_party_master_csv, sap_person_db; notebook uses combined, crm_party_master_csv, sap_person_db.
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `legal_person_hash_key` source `md5(concat(person_id, origin_sys))` vs notebook `md5(concat(origin_sys, person_id))`; `load_date` source `cast(current_timestamp() as timestamp)` vs notebook `cast(pull_ts as timestamp)`.
...
- FROM tables differ: source uses crm_enhanced_enrichments_csv; notebook uses crm_campaign_touch_csv.
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`; `marketing_engagement_hash_key` source `md5(concat(src_marketing_engagement_ref, origin_sys))` vs notebook `md5(concat(origin_sys, engagement_ref))`; `marketing_engagement_id` source `src_marketing_engagement_ref` vs notebook `engagement_ref`.
...
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`; `marketing_preference_hash_key` source `md5(concat(preference_ref, origin_sys))` vs notebook `md5(concat(origin_sys, preference_ref))`.
...
- FROM tables differ: source uses crm_vehicle_asset_csv, sap_motor_db; notebook uses combined, crm_vehicle_asset_csv, sap_motor_db.
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`; `motor_hash_key` source `md5(concat(motor_id, origin_sys))` vs notebook `md5(concat(origin_sys, motor_id))`.
...
- FROM tables differ: source uses crm_party_master_csv, sap_person_db; notebook uses combined, crm_party_master_csv, sap_person_db.
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`; `natural_person_hash_key` source `md5(concat(person_id, origin_sys))` vs notebook `md5(concat(origin_sys, person_id))`.
...
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`; `override_hash_key` source `md5(concat(src_override_id, src_system))` vs notebook `md5(concat(origin_sys, src_override_id))`; `record_source` source `src_system` vs notebook `origin_sys`.
...
- FROM tables differ: source uses crm_party_master_csv, sap_person_db; notebook uses combined, crm_party_master_csv, sap_person_db.
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`; `person_hash_key` source `md5(concat(person_id, origin_sys))` vs notebook `md5(concat(origin_sys, person_id))`.
...
- FROM tables differ: source uses crm_policy_register_csv; notebook uses combined, crm_policy_register_csv, sap_home_db, sap_motor_db.
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`; `policy_hash_key` source `md5(concat(policy_ref, origin_sys))` vs notebook `md5(concat(origin_sys, policy_id))`; `policy_id` source `policy_ref` vs notebook `policy_id`.
...
- FROM tables differ: source uses crm_product_catelog_csv, sap_product_db; notebook uses combined, crm_product_catalog_csv, sap_home_db, sap_motor_db, sap_product_db.
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`; `product_hash_key` source `md5(concat(product_id, origin_sys))` vs notebook `md5(concat(origin_sys, product_id))`.
...
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`; `quote_hash_key` source `md5(concat(quote_ref, origin_sys))` vs notebook `md5(concat(quote_ref, quote_ref))`.
...
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`; `record_source` source `src_system` vs notebook `origin_sys`; `regulation_hash_key` source `md5(concat(src_regulation_id, src_system))` vs notebook `md5(concat(origin_sys, src_regulation_id))`.
...
- FROM tables differ: source uses combined, crm_broker_book_csv; notebook uses crm_enhanced_person_relationships_csv.
- Notebook removes output columns: `broker_person_hash_key`, `load_ts`.
- Changed expressions: `broker_hash_key` source `md5(concat(src_agent_id, src_system))` vs notebook `md5(concat(origin_sys, src_agent_id))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `person_hash_key` source `md5(concat(party_ref, origin_sys))` vs notebook `md5(concat(origin_sys, src_person_id))`; `record_source` source `coalesce(src_system, origin_sys)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_claim_register_csv, crm_policy_register_csv; notebook uses crm_enhanced_policy_relationships_csv.
- Notebook removes output columns: `claim_policy_hash_key`, `etl_load_timestamp`.
- Changed expressions: `claim_hash_key` source `cast(null as string)` vs notebook `md5(concat(origin_sys, src_claim_id))`; `load_date` source `cast(etl_load_timestamp as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `policy_hash_key` source `md5(concat(policy_ref, origin_sys))` vs notebook `md5(concat(origin_sys, src_policy_id))`; `record_source` source `record_source` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_complaint_register_csv; notebook uses crm_enhanced_policy_relationships_csv.
- Notebook removes output columns: `complaint_policy_hash_key`.
- Changed expressions: `complaint_hash_key` source `md5(concat(src_complaint_id, src_system))` vs notebook `md5(concat(origin_sys, src_complaint_id))`; `load_date` source `cast(load_date as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `policy_hash_key` source `md5(concat(policy_ref, origin_sys))` vs notebook `md5(concat(origin_sys, src_policy_id))`; `record_source` source `record_source` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_complaint_register_csv, crm_regulation_register_csv; notebook uses crm_enhanced_policy_relationships_csv.
- Notebook removes output columns: `complaint_regulation_hash_key`, `load_ts`, `origin_sys`, `source_tbl_name`.
- Changed expressions: `complaint_hash_key` source `md5(concat(src_complaint_id, src_system))` vs notebook `md5(concat(origin_sys, src_complaint_id))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `record_source` source `coalesce(origin_sys, origin_sys)` vs notebook `origin_sys`; `regulation_hash_key` source `md5(concat(src_regulation_id, src_system))` vs notebook `md5(concat(origin_sys, src_regulation_id))`.
...
- FROM tables differ: source uses combined, crm_customer_portfolio_csv, crm_lead_register_csv; notebook uses crm_customer_lead_bridge_csv.
- Notebook removes output columns: `customer_lead_hash_key`, `load_ts`, `source_tbl_name`.
- Changed expressions: `customer_hash_key` source `md5(concat(customer_ref, origin_sys))` vs notebook `md5(concat(origin_sys,customer_ref))`; `lead_hash_key` source `md5(concat(lead_ref, origin_sys))` vs notebook `md5(concat(origin_sys, lead_ref))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `record_source` source `coalesce(origin_sys, origin_sys)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_customer_portfolio_csv, crm_party_master_csv; notebook uses crm_customer_portfolio_csv.
- Notebook removes output columns: `customer_person_hash_key`, `load_ts`, `source_tbl_name`.
- Changed expressions: `customer_hash_key` source `md5(concat(customer_ref, origin_sys))` vs notebook `md5(concat(origin_sys,customer_ref))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `person_hash_key` source `md5(concat(party_ref, origin_sys))` vs notebook `md5(concat(origin_sys, party_ref))`; `record_source` source `coalesce(origin_sys, origin_sys)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_property_asset_csv; notebook uses crm_enhanced_policy_relationships_csv.
- Notebook removes output columns: `insured_object_home_hash_key`, `load_timestamp`, `source_tbl_name`.
- Changed expressions: `home_hash_key` source `md5(concat(property_ref, origin_sys))` vs notebook `md5(concat(origin_sys, src_home_id))`; `insured_object_hash_key` source `md5(concat(insured_object_id, origin_sys))` vs notebook `md5(concat(origin_sys, src_insured_object_id))`; `load_date` source `cast(load_timestamp as timestamp)` vs notebook `cast(pull_ts as timestamp)`.
...
- FROM tables differ: source uses combined, crm_vehicle_asset_csv; notebook uses crm_enhanced_policy_relationships_csv.
- Notebook removes output columns: `insured_object_motor_hash_key`, `load_timestamp`, `source_tbl_name`.
- Changed expressions: `insured_object_hash_key` source `md5(concat(insured_object_id, origin_sys))` vs notebook `md5(concat(origin_sys, src_insured_object_id))`; `load_date` source `cast(load_timestamp as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `motor_hash_key` source `md5(concat(vehicle_ref, origin_sys))` vs notebook `md5(concat(origin_sys, src_motor_id))`.
...
- FROM tables differ: source uses combined, crm_party_master_csv; notebook uses crm_account_book_csv.
- Notebook removes output columns: `load_ts`, `person_account_hash_key`, `source_tbl_name`.
- Changed expressions: `account_hash_key` source `sha2(concat_ws('||', cast(account_ref as string), cast(origin_sys as string)), 256)` vs notebook `md5(concat(origin_sys,account_ref))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `person_hash_key` source `sha2(concat_ws('||', cast(party_ref as string), cast(origin_sys as string)), 256)` vs notebook `md5(concat(origin_sys, party_ref))`; `record_source` source `coalesce(origin_sys, origin_sys)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_party_master_csv; notebook uses combined, crm_address_book_csv, crm_enhanced_person_relationships_csv, sap_address_db.
- Notebook removes output columns: `load_ts`, `person_address_hash_key`, `source_tbl_name`.
- Changed expressions: `address_hash_key` source `md5(concat(address_ref, origin_sys))` vs notebook `md5(concat(origin_sys,address_id))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `person_hash_key` source `md5(concat(party_ref, origin_sys))` vs notebook `md5(concat(origin_sys, person_id))`; `record_source` source `coalesce(origin_sys, origin_sys)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_party_master_csv; notebook uses crm_enhanced_person_relationships_csv.
- Notebook removes output columns: `load_timestamp`, `person_campaign_hash_key`.
- Changed expressions: `campaign_hash_key` source `md5(concat(src_campaign_id, src_system))` vs notebook `md5(concat(origin_sys, src_campaign_id))`; `load_date` source `cast(load_timestamp as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `person_hash_key` source `md5(concat(party_ref, origin_sys))` vs notebook `md5(concat(origin_sys, src_person_id))`; `record_source` source `coalesce(origin_sys, src_system)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_party_master_csv; notebook uses crm_consent_snapshot_csv.
- Notebook removes output columns: `load_ts`, `person_consent_hash_key`, `source_tbl_name`.
- Changed expressions: `consent_hash_key` source `md5(concat(consent_ref, origin_sys))` vs notebook `md5(concat(origin_sys,consent_ref))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `person_hash_key` source `md5(concat(party_ref, origin_sys))` vs notebook `md5(concat(origin_sys, party_ref))`; `record_source` source `coalesce(origin_sys, origin_sys)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_contact_point_csv, crm_party_master_csv; notebook uses crm_contact_point_csv.
- Notebook removes output columns: `load_ts`, `person_contact_hash_key`, `source_tbl_name`.
- Changed expressions: `contact_hash_key` source `md5(concat(contact_ref, origin_sys))` vs notebook `md5(concat(origin_sys,contact_ref))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `person_hash_key` source `md5(concat(party_ref, origin_sys))` vs notebook `md5(concat(origin_sys, party_ref))`; `record_source` source `coalesce(origin_sys, origin_sys)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_identity_registry_csv, crm_party_master_csv; notebook uses crm_identity_registry_csv.
- Notebook removes output columns: `person_identities_hash_key`, `source_load_timestamp_col`, `source_tbl_name`.
- Changed expressions: `identities_hash_key` source `md5(concat(identity_ref, origin_sys))` vs notebook `md5(concat(origin_sys,identity_ref))`; `load_date` source `cast(source_load_timestamp_col as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `person_hash_key` source `md5(concat(person_id, origin_sys))` vs notebook `md5(concat(origin_sys, party_ref))`; `record_source` source `coalesce(origin_sys, origin_sys)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_party_master_csv; notebook uses crm_lead_register_csv.
- Notebook removes output columns: `load_ts`, `person_lead_hash_key`, `source_tbl_name`.
- Changed expressions: `lead_hash_key` source `md5(concat(lead_ref, origin_sys))` vs notebook `md5(concat(origin_sys,lead_ref))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `person_hash_key` source `md5(concat(party_ref, origin_sys))` vs notebook `md5(concat(origin_sys, party_ref))`.
...
- Notebook removes output columns: `etl_load_timestamp`, `person_legal_person_hash_key`, `source_tbl_name`.
- Changed expressions: `legal_person_hash_key` source `md5(concat(person_id, origin_sys))` vs notebook `md5(concat(origin_sys, person_id))`; `load_date` source `cast(etl_load_timestamp as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `person_hash_key` source `md5(concat(person_id, origin_sys))` vs notebook `md5(concat(origin_sys, person_id))`.
...
- FROM tables differ: source uses combined, crm_enhanced_enrichments_csv, crm_party_master_csv; notebook uses crm_campaign_touch_csv.
- Notebook removes output columns: `etl_load_timestamp`, `person_marketing_engagement_hash_key`, `source_tbl_name`.
- Changed expressions: `load_date` source `cast(etl_load_timestamp as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `marketing_engagement_hash_key` source `md5(concat(src_marketing_engagement_ref, origin_sys))` vs notebook `md5(concat(origin_sys,engagement_ref))`; `person_hash_key` source `md5(concat(party_ref, origin_sys))` vs notebook `md5(concat(origin_sys, party_ref))`; `record_source` source `coalesce(origin_sys, origin_sys)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_party_master_csv; notebook uses crm_comm_preference_csv.
- Notebook removes output columns: `etl_load_timestamp`, `person_marketing_preference_hash_key`, `source_tbl_name`.
- Changed expressions: `load_date` source `cast(etl_load_timestamp as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `marketing_preference_hash_key` source `md5(concat(preference_ref, origin_sys))` vs notebook `md5(concat(origin_sys,preference_ref))`; `person_hash_key` source `md5(concat(party_ref, origin_sys))` vs notebook `md5(concat(origin_sys, party_ref))`; `record_source` source `coalesce(origin_sys, origin_sys)` vs notebook `origin_sys`.
...
- Notebook removes output columns: `person_natural_person_hash_key`, `source_load_timestamp_col`, `source_tbl_name`.
- Changed expressions: `load_date` source `cast(source_load_timestamp_col as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `natural_person_hash_key` source `sha2(concat_ws('||', cast(person_id as string), cast(origin_sys as string)), 256)` vs notebook `md5(concat(origin_sys, person_id))`; `person_hash_key` source `sha2(concat_ws('||', cast(person_id as string), cast(origin_sys as string)), 256)` vs notebook `md5(concat(origin_sys, person_id))`.
...
- FROM tables differ: source uses combined, crm_broker_book_csv, crm_policy_register_csv; notebook uses crm_enhanced_policy_relationships_csv.
- Notebook removes output columns: `load_ts`, `policy_broker_hash_key`.
- Changed expressions: `broker_hash_key` source `sha2(concat_ws('||', cast(src_agent_id as string), cast(src_system as string)), 256)` vs notebook `md5(concat(origin_sys, src_agent_id))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `policy_hash_key` source `sha2(concat_ws('||', cast(policy_ref as string), cast(origin_sys as string)), 256)` vs notebook `md5(concat(origin_sys, src_policy_id))`; `record_source` source `coalesce(origin_sys, src_system)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_policy_register_csv; notebook uses crm_enhanced_policy_relationships_csv.
- Notebook removes output columns: `load_ts`, `policy_channel_hash_key`.
- Changed expressions: `channel_hash_key` source `md5(concat(src_channel_id, src_system))` vs notebook `md5(concat(origin_sys, src_channel_id))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `policy_hash_key` source `md5(concat(policy_ref, origin_sys))` vs notebook `md5(concat(origin_sys, src_policy_id))`; `record_source` source `coalesce(origin_sys, src_system)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_customer_portfolio_csv, crm_policy_register_csv; notebook uses crm_policy_register_csv.
- Notebook removes output columns: `load_ts`, `policy_customer_hash_key`, `source_tbl_name`.
- Changed expressions: `customer_hash_key` source `md5(concat(customer_ref, origin_sys))` vs notebook `md5(concat(origin_sys,customer_ref))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `policy_hash_key` source `md5(concat(policy_ref, origin_sys))` vs notebook `md5(concat(origin_sys, policy_ref))`; `record_source` source `coalesce(origin_sys, origin_sys)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_policy_register_csv, crm_property_asset_csv, crm_vehicle_asset_csv; notebook uses crm_enhanced_policy_relationships_csv.
- Notebook removes output columns: `policy_insured_object_hash_key`, `source_load_timestamp`, `source_tbl_name`.
- Changed expressions: `insured_object_hash_key` source `md5(concat(insured_object_id, origin_sys))` vs notebook `md5(concat(origin_sys, src_insured_object_id))`; `load_date` source `cast(source_load_timestamp as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `policy_hash_key` source `md5(concat(policy_ref, origin_sys))` vs notebook `md5(concat(origin_sys, src_policy_id))`; `record_source` source `coalesce(origin_sys, origin_sys)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_override_register_csv, crm_policy_register_csv; notebook uses crm_enhanced_policy_relationships_csv.
- Notebook removes output columns: `policy_override_hash_key`, `source_load_timestamp`.
- Changed expressions: `load_date` source `cast(source_load_timestamp as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `override_hash_key` source `md5(concat(src_override_id, src_system))` vs notebook `md5(concat(origin_sys, src_override_id))`; `policy_hash_key` source `md5(concat(policy_ref, origin_sys))` vs notebook `md5(concat(origin_sys, src_policy_id))`; `record_source` source `coalesce(origin_sys, src_system)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_policy_register_csv; notebook uses combined, crm_policy_register_csv, sap_home_db, sap_motor_db.
- Notebook removes output columns: `etl_load_timestamp`, `policy_product_hash_key`, `source_tbl_name`.
- Changed expressions: `load_date` source `cast(etl_load_timestamp as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `policy_hash_key` source `md5(concat(policy_ref, origin_sys))` vs notebook `md5(concat(origin_sys, policy_id))`; `product_hash_key` source `md5(concat(product_ref, origin_sys))` vs notebook `md5(concat(origin_sys, product_id))`; `record_source` source `coalesce(origin_sys, origin_sys)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_policy_register_csv, crm_quote_register_csv; notebook uses combined, crm_enhanced_policy_relationships_csv, crm_policy_register_csv.
- Notebook removes output columns: `policy_quote_hash_key`, `source_load_timestamp_col`, `source_tbl_name`.
- Changed expressions: `load_date` source `cast(source_load_timestamp_col as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `policy_hash_key` source `md5(concat(policy_ref, origin_sys))` vs notebook `md5(concat(origin_sys, src_policy_id))`; `quote_hash_key` source `md5(concat(quote_ref, origin_sys))` vs notebook `md5(concat(origin_sys, src_quote_id))`; `record_source` source `coalesce(origin_sys, origin_sys)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_broker_book_csv, crm_quote_register_csv; notebook uses crm_enhanced_policy_relationships_csv.
- Notebook removes output columns: `load_ts`, `quote_broker_hash_key`.
- Changed expressions: `broker_hash_key` source `md5(concat(src_agent_id, src_system))` vs notebook `md5(concat(origin_sys, src_agent_id))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `quote_hash_key` source `md5(concat(quote_ref, origin_sys))` vs notebook `md5(concat(origin_sys, src_quote_id))`; `record_source` source `coalesce(origin_sys, src_system)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_quote_register_csv; notebook uses crm_enhanced_policy_relationships_csv.
- Notebook removes output columns: `quote_channel_hash_key`, `source_load_timestamp_col`.
- Changed expressions: `channel_hash_key` source `sha2(concat_ws('||', cast(src_channel_id as string), cast(src_system as string)), 256)` vs notebook `md5(concat(origin_sys, src_channel_id))`; `load_date` source `cast(source_load_timestamp_col as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `quote_hash_key` source `sha2(concat_ws('||', cast(quote_ref as string), cast(origin_sys as string)), 256)` vs notebook `md5(concat(origin_sys, src_quote_id))`; `record_source` source `coalesce(origin_sys, src_system)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_party_master_csv, crm_quote_register_csv; notebook uses crm_quote_register_csv.
- Notebook removes output columns: `load_ts`, `quote_person_hash_key`, `source_tbl_name`.
- Changed expressions: `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `person_hash_key` source `md5(concat(party_ref, origin_sys))` vs notebook `md5(concat(origin_sys, party_ref))`; `quote_hash_key` source `md5(concat(quote_ref, origin_sys))` vs notebook `md5(concat(origin_sys,quote_ref))`; `record_source` source `coalesce(origin_sys, origin_sys)` vs notebook `origin_sys`.
...
- FROM tables differ: source uses combined, crm_product_catelog_csv, crm_quote_register_csv; notebook uses crm_quote_register_csv.
- Notebook removes output columns: `etl_load_timestamp`, `quote_product_hash_key`, `source_tbl_name`.
- Changed expressions: `load_date` source `cast(etl_load_timestamp as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `product_hash_key` source `md5(concat(product_ref, origin_sys))` vs notebook `md5(concat(origin_sys, product_ref))`; `quote_hash_key` source `md5(concat(quote_ref, origin_sys))` vs notebook `md5(concat(origin_sys,quote_ref))`; `record_source` source `coalesce(origin_sys, origin_sys)` vs notebook `origin_sys`.
...
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `account_hash_key` source `md5(concat(account_ref, origin_sys))` vs notebook `md5(concat(origin_sys, account_ref))`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`.
...
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `address_hash_key` source `md5(concat(address_ref, origin_sys))` vs notebook `md5(concat(origin_sys, address_ref))`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`.
...
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `address_hash_key` source `md5(concat(address_id, origin_sys))` vs notebook `md5(concat(origin_sys, address_id))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`.
...
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `broker_hash_key` source `md5(concat(src_agent_id, src_system))` vs notebook `md5(concat(origin_sys, src_agent_id))`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`.
...
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `campaign_hash_key` source `md5(concat(src_campaign_id, src_system))` vs notebook `md5(concat(origin_sys, src_campaign_id))`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`.
...
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `channel_hash_key` source `md5(concat(src_channel_id, src_system))` vs notebook `md5(concat(origin_sys, src_channel_id))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`.
...
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `claim_hash_key` source `md5(concat(src_claim_id, src_system))` vs notebook `md5(concat(origin_sys, src_claim_id))`; `claim_reported_date` source `to_timestamp(src_claim_reported_date)` vs notebook `src_claim_reported_date`; `claim_settlement_date` source `to_timestamp(src_claim_settlement_date)` vs notebook `src_claim_settlement_date`; `first_recovery_date` source `to_timestamp(src_first_recovery_date)` vs notebook `src_first_recovery_date`; `last_recovery_date` source `to_timestamp(src_last_recovery_date)` vs notebook `src_last_recovery_date`; `litigation_end_date` source `to_timestamp(src_litigation_end_date)` vs notebook `src_litigation_end_date`; `litigation_start_date` source `to_timestamp(src_litigation_start_date)` vs notebook `src_litigation_start_date`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`.
...
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `complaint_hash_key` source `md5(concat(src_complaint_id, src_system))` vs notebook `md5(concat(origin_sys, src_complaint_id))`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`.
...
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `contact_hash_key` source `md5(concat(contact_ref, origin_sys))` vs notebook `md5(concat(origin_sys, contact_ref))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`.
...
- FROM tables differ: source uses crm_customer_portfolio_csv, crm_enhanced_enrichments_csv; notebook uses combined, crm_customer_portfolio_csv.
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `customer_age_band` source `src_customer_age_band` vs notebook `p.src_customer_age_band`; `customer_hash_key` source `md5(concat(customer_ref, origin_sys))` vs notebook `md5(concat(s.origin_sys, s.customer_ref))`; `customer_number` source `null` vs notebook `s.customer_no`; `customer_onboarding_feedback` source `null` vs notebook `s.customer_onboarding_feedback`; `customer_onboarding_satisfaction_score` source `null` vs notebook `s.customer_onboarding_satisfaction_score`; `customer_rating` source `null` vs notebook `s.customer_rating_no`; `customer_satisfaction` source `src_customer_satisfaction` vs notebook `p.src_customer_satisfaction`; `customer_segment` source `null` vs notebook `s.customer_segment_txt` plus 8 more.
...
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `home_hash_key` source `md5(concat(property_ref, origin_sys))` vs notebook `md5(concat(origin_sys, property_ref))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`.
...
- FROM tables differ: source uses sap_home_db, sap_motor_db; notebook uses sap_home_db.
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `home_hash_key` source `md5(concat(policy_id, origin_sys))` vs notebook `md5(concat(origin_sys, home_id))`; `home_location` source `null` vs notebook `home_location`; `home_type` source `null` vs notebook `home_type`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `policy_id` source `null` vs notebook `policy_id`; `product_id` source `null` vs notebook `product_id`; `roof_material` source `null` vs notebook `roof_material`; `wall_type` source `null` vs notebook `wall_type`.
...
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `identities_hash_key` source `md5(concat(identity_ref, origin_sys))` vs notebook `md5(concat(origin_sys, identity_ref))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`.
...
- FROM tables differ: source uses crm_property_asset_csv, crm_vehicle_asset_csv; notebook uses combined, crm_property_asset_csv, crm_vehicle_asset_csv.
- Notebook adds output columns: `currency_code`, `insured_object_current_status`, `insured_object_description`, `insured_object_end_date`, `insured_object_start_date`, `insured_object_sub_type`, `insured_object_type`, `insured_value`.
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `insured_object_hash_key` source `md5(concat(insured_object_id, origin_sys))` vs notebook `md5(concat(origin_sys, insured_object_id))`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`.
...
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `lead_hash_key` source `md5(concat(lead_ref, origin_sys))` vs notebook `md5(concat(origin_sys, lead_ref))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`.
...
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `converted_date` source `lead_conv_dt` vs notebook `cast(lead_conv_dt as timestamp)`; `date_of_constitution` source `constitution_dt` vs notebook `to_date(constitution_dt,'y-m-d')`; `legal_person_hash_key` source `md5(concat(legal_ref, origin_sys))` vs notebook `md5(concat(origin_sys, legal_ref))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`.
...
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `legal_person_hash_key` source `md5(concat(person_id, origin_sys))` vs notebook `md5(concat(origin_sys, person_id))`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `org_establishment_date` source `org_establishment_date` vs notebook `to_date(org_establishment_date, 'y-m-d')`.
...
- FROM tables differ: source uses crm_campaign_touch_csv, crm_enhanced_enrichments_csv; notebook uses crm_campaign_touch_csv.
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `marketing_engagement_hash_key` source `md5(concat(src_marketing_engagement_ref, origin_sys))` vs notebook `md5(concat(origin_sys, engagement_ref))`; `marketing_status` source `null` vs notebook `campaign_status_txt`; `opened_email` source `null` vs notebook `email_opened_ind`; `promotion_code` source `null` vs notebook `promo_cd`.
...
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `marketing_preference_hash_key` source `md5(concat(preference_ref, origin_sys))` vs notebook `md5(concat(origin_sys, preference_ref))`.
...
- FROM tables differ: source uses crm_enhanced_enrichments_csv, crm_vehicle_asset_csv; notebook uses crm_vehicle_asset_csv.
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `auto_decline_vehicle` source `cast(null as string)` vs notebook `auto_decline_ind`; `body_type` source `cast(null as string)` vs notebook `body_style_txt`; `driver_experience_years` source `cast(null as bigint)` vs notebook `driver_experience_years`; `fuel_type` source `cast(null as string)` vs notebook `fuel_type_txt`; `is_existing_motor_customer` source `cast(null as string)` vs notebook `existing_motor_ind`; `license_status` source `cast(null as string)` vs notebook `license_status_txt`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `motor_hash_key` source `md5(concat(cast(vehicle_ref as string), cast(origin_sys as string)))` vs notebook `md5(concat(origin_sys, vehicle_ref))` plus 12 more.
...
- Notebook adds output columns: `body_colour`, `fuel_type`, `gear_type`, `manufacturing_date`, `motor_class`, `motor_model`, `motor_parked_location`, `motor_type`, `policy_id`, `product_id`.
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `motor_hash_key` source `md5(concat(motor_id, origin_sys))` vs notebook `md5(concat(origin_sys, motor_id))`.
...
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `birth_date` source `dob` vs notebook `to_date(dob, 'y-m-d')`; `birth_year` source `birth_yr` vs notebook `cast(birth_yr as int)`; `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `natural_person_hash_key` source `md5(concat(natural_ref, origin_sys))` vs notebook `md5(concat(origin_sys, natural_ref))`.
...
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `date_of_birth` source `date_of_birth` vs notebook `to_date(date_of_birth, 'y-m-d')`; `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`; `natural_person_hash_key` source `md5(concat(person_id, origin_sys))` vs notebook `md5(concat(origin_sys, person_id))`.
...
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`; `override_hash_key` source `md5(concat(src_override_id, src_system))` vs notebook `md5(concat(origin_sys, src_override_id))`.
...
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`; `person_hash_key` source `md5(concat(party_ref, origin_sys))` vs notebook `md5(concat(origin_sys, party_ref))`.
...
- Notebook adds output columns: `email_address`, `person_type`, `phone_number`.
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `person_hash_key` source `md5(concat(person_id, origin_sys))` vs notebook `md5(concat(origin_sys, person_id))`.
...
- FROM tables differ: source uses crm_enhanced_enrichments_csv, crm_policy_register_csv; notebook uses combined, crm_policy_register_csv.
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `administrative_expenses` source `src_administrative_expenses` vs notebook `p.src_administrative_expenses`; `amount_approved` source `src_amount_approved` vs notebook `p.src_amount_approved`; `ceded_commission` source `src_ceded_commission` vs notebook `p.src_ceded_commission`; `ceded_premium` source `src_ceded_premium` vs notebook `p.src_ceded_premium`; `commission_paid` source `src_commission_paid` vs notebook `p.src_commission_paid`; `cover_option` source `null` vs notebook `s.cover_option_txt`; `declined_claims` source `null` vs notebook `s.declined_claim_cnt`; `discount` source `src_discount` vs notebook `p.src_discount` plus 51 more.
...
- FROM tables differ: source uses crm_product_catalog_csv, crm_product_catelog_csv; notebook uses crm_product_catalog_csv.
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `product_hash_key` source `md5(concat(product_ref, origin_sys))` vs notebook `md5(concat(origin_sys, product_ref))`.
...
- Notebook adds output columns: `line_of_business`, `product_name`, `product_start_date`, `product_sub_type`, `product_type`.
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `load_date` source `cast(load_ts as timestamp)` vs notebook `cast(pull_ts as timestamp)`; `product_hash_key` source `md5(concat(cast(product_id as string), cast(origin_sys as string)))` vs notebook `md5(concat(origin_sys, product_id))`.
...
- Notebook removes output columns: `source_tbl_name`.
- Changed expressions: `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`; `quote_hash_key` source `md5(concat(quote_ref, origin_sys))` vs notebook `md5(concat(origin_sys, quote_ref))`.
...
- Notebook removes output columns: `record_source`, `source_tbl_name`.
- Changed expressions: `load_date` source `current_timestamp()` vs notebook `cast(pull_ts as timestamp)`; `regulation_hash_key` source `md5(concat(src_regulation_id, src_system))` vs notebook `md5(concat(origin_sys, src_regulation_id))`.
